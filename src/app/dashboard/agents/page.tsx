...
// Abilities offered per agent type
...
const CHAT_CHANNELS = ["whatsapp", "instagram", "messenger", "sms", "email", "tiktok"] as const;
...
export default function AgentsPage() {
  ...
  const visible = tab === "chat" ? agents.filter((a) => a.kind === "chat")
    : tab === "voice" ? agents.filter((a) => a.kind === "voice")
    : agents;
  ...
  return (
    <>
      ...
      {source === "live" ? (
        ...
        <DemoBanner context="Agents table not found — run supabase/migrations/0002_agents.sql (and 0003) in the SQL Editor." />
      )}
      ...
      <div className="mb-5 flex gap-1 rounded-xl border border-ink-200 bg-surface p-1">
        {(
          [
            { key: "all", label: "All agents", icon: Bot },
            { key: "chat", label: "Chat agents", icon: MessageCircle },
            { key: "voice", label: "Voice agents", icon: PhoneCall },
            { key: "hub", label: "Agent Hub", icon: Sparkles },
          ] as const
        ).map((t) => (
          <button
            key={t.key}
              ...
              <div className="mt-4 flex gap-2 border-t border-ink-100 pt-4">
                {a.kind === "chat" ? (
                  ...
                  <span className="flex flex-1 items-center justify-center rounded-xl border border-ink-200 py-2 text-sm text-ink-500">
                    Runs on Vapi — connect a phone line in the Agent Hub
                  </span>
                )}
                <button
...
function AgentHub({ agents }: { agents: AiAgent[] }) {
...
function AgentModal({
  ...
  // Extracted text per uploaded document, merged into the knowledge base on save
  ...
  async function submit() {
    ...
    setSaving(true);
    // Knowledge base = existing text (when editing) + all uploaded document text
    ...
    if (res.ok && form.kind === "voice") {
      try {
        ...
        if (vapiRes.ok && vapiData.id && res.id) await setAgentVapiId(res.id, vapiData.id);
        message = vapiRes.ok
          ? `Agent saved${initial ? "" : " and created in Vapi"}.`
    ...
    >
      {result?.ok ? (
        ...
        <>
          ...
          {/* Knowledge base: document upload (max 10) */}
          <div className="mt-4">
            ...
            {initial && form.knowledgeBase && form.kbFiles.length === 0 && (
              <p className="mt-2 text-xs text-ink-400">
                This agent has an existing text knowledge base — uploading documents adds to it.
              </p>
            )}
          </div>
