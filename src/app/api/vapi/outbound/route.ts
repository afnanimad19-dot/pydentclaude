...
export async function POST(req: NextRequest) {
  ...
  const { assistantId, fromNumber, numbers } = await req.json().catch(() => ({}));
  ...
  if (!fromNumber) return NextResponse.json({ ok: false, error: "The campaign has no phone number to call from." }, { status: 400 });
  ...
  // Find the Vapi phone-number id that matches the campaign's from-number.
  let phoneNumberId = "";
  try {
