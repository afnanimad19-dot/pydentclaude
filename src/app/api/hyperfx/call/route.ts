...
import { getHfxCreds, hfxCall, hfxConfigured, hfxToolIsSafe } from "@/lib/hyperfx";
...
export async function POST(req: NextRequest) {
  let body: { tool?: string; args?: Record<string, unknown>; ws?: string };
  try {
    body = await req.json();
  } catch {
  ...
  if (!hfxConfigured(creds)) {
    return NextResponse.json({ error: "Hyperfx is not configured — save the clinic's Hyperfx credentials in Settings → Connections, or set HYPERFX_MCP_URL / HYPERFX_API_KEY in Netlify." }, { status: 400 });
  }
  ...
  const r = await hfxCall(tool, body.args ?? {}, creds);
